...
viz_type = st.selectbox('Select metrics',
                       ['Online Donations with Forecast'
                        ...
                        , 'Giving Trends'
                        , 'Average Gift by Age'
                        , 'Online Preference by Age'
                        , 'Online Donation Frequency'])

if viz_type == 'Online vs Offline Distribution':
    online_counts = df['Online'].value_counts()
    fig = go.Figure(data=[go.Pie(labels=['Offline', 'Online'],
                                values=online_counts,
                                hole=0.3)])
    fig.update_layout(
        title='Proportion of Online vs Offline Donations',
    ...
    fig = go.Figure(data=[go.Pie(labels=['Offline', 'Online'],
                                ...
                                hole=0.3)])
    fig.update_layout(
        title='Proportion of Gift Amount: Online vs Offline',
        template='plotly_white')
...
elif viz_type == 'Key Indicator Categories':
    ...
    for col in key_indicator_online_pct.columns:
        fig.add_trace(go.Bar(
            name='Online' if col == 1 else 'Offline',
            x=key_indicator_online_pct.index,
            y=key_indicator_online_pct[col],
        ))
    fig.update_layout(
        barmode='stack',
        title='Key Indicator Categories for Online vs Offline Donations',
    ...
    fig.update_layout(
        title='Distribution of Donor Age',
        xaxis_title='Age',
        yaxis_title='Count'
    )
    ...
    fig.update_layout(
        title='Distribution of Donors by Age Group',
        xaxis_title='Age Group',
        yaxis_title='Count'
    )

elif viz_type == 'Giving Trends':
    yearly_giving = df[['FY18 Giving', 'FY19 Giving', 'FY20 Giving', 
                       'FY21 Giving', 'FY22 Giving', 'FY23 Giving', 'FY24 Giving']]
    yearly_giving = yearly_giving.rename(columns={
        'FY18 Giving': 'FY18', 'FY19 Giving': 'FY19',
    ...
    fig.update_layout(
        title='Total Giving by Fiscal Year',
        xaxis_title='Fiscal Year',
        yaxis_title='Total Amount'
    )

elif viz_type == 'Average Gift by Age':
    avg_gift_by_age = df.groupby('AgeGroup')['Fund Split Amount'].mean()
    fig = go.Figure(data=[go.Bar(
        x=avg_gift_by_age.index,
        y=avg_gift_by_age.values
    )])
    fig.update_layout(
        title='Average Gift Amount by Age Group',
        xaxis_title='Age Group',
        yaxis_title='Average Gift Amount'
    )

elif viz_type == 'Online Preference by Age':
    online_by_age = df.groupby('AgeGroup')['Online'].mean().sort_values(ascending=False)
    fig = go.Figure(data=[go.Bar(
        x=online_by_age.index,
        y=online_by_age.values
    )])
    fig.update_layout(
        title='Proportion of Online Donations by Age Group',
        xaxis_title='Age Group',
    ...
    donation_frequency['frequency_bin'] = pd.cut(
        donation_frequency['count'],
        bins=[0, 1, 2, 5, 10, 20, 50, 100, float('inf')],
        labels=['1', '2', '3-5', '6-10', '11-20', '21-50', '51-100', '100+']
    )
    fig = go.Figure(data=[go.Box(
        x=donation_frequency['frequency_bin'],
        y=donation_frequency['mean']
    )])
    fig.update_layout(
        title='Distribution of Online Donation Proportion by Donation Frequency',
